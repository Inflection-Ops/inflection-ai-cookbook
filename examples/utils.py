...
def parse_xml_response(xml_string: str, keys_to_search: list) -> Dict[str, object]:
    ...
    for k in keys_to_search:
        value = re.search(rf"<{k}>(.+?)</{k}>", xml_string).group(1)
        if k == "participants":
            value = [participant.strip() for participant in value.strip('[]').split(',')]
        result[k] = value
    return result

async def get_response(context, keys) -> Dict[str, object]:
    result = await fetch_inflection(context, model="inflection_3_productivity")
